...
function Links({ isOpen, setIsOpen }: LinksProps) {
  ...
  return (
    <div
      className={`absolute flex flex-col gap-y-8 pt-10 md:pt-0
       md:flex-row items-center w-full md:w-auto h-screen md:h-auto
        md:relative bg-white md:left-auto text-xl transition-all
         ${isOpen ? "-left-[150%]" : "left-0"} top-0`}
    >
      {renderLinks()}
      <button
        className="md:hidden absolute right-3 top-3 text-neutral-500"
